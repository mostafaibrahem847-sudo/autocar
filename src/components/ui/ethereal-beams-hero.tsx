...
function extendMaterial<T extends THREE.Material = THREE.Material>(
  ...
  const uniforms = THREE.UniformsUtils.clone(baseUniforms) as Record<string, THREE.IUniform>;
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, uniformValue]) => {
    uniforms[key] =
      uniformValue !== null && typeof uniformValue === "object" && "value" in uniformValue
        ? (uniformValue as THREE.IUniform<unknown>)
        : ({ value: uniformValue } as THREE.IUniform<unknown>);
  });
...
>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    if (mesh.current.material.uniforms.time) {
      mesh.current.material.uniforms.time.value = Number(mesh.current.material.uniforms.time.value) + 0.1 * delta;
    }
  });
...
const HeroButton = ({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 -bottom-2 -top-2 -translate-x-full skew-x-12 bg-gradient-to-r from-transparent via-white/20 to-transparent transition-transform duration-1000 ease-out group-hover:translate-x-full" />
    </button>
  );
};
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <HeroButton variant="ghost" size="sm" className="hidden sm:flex">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <div className="mb-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
              <HeroButton size="lg" className="font-semibold shadow-2xl shadow-white/25">
                Start Creating
                <ArrowRight className="ml-2 h-5 w-5" />
              </HeroButton>
              <HeroButton variant="outline" size="lg" className="bg-transparent font-semibold">
                Learn More
              </HeroButton>
            </div>
